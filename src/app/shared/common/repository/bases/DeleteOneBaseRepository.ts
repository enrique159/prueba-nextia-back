...
import { IDeleteOneBaseRepository } from '..'
...
export default class DeleteOneBaseRepository<T, U> implements IDeleteOneBaseRepository<T, U> {
  ...
  public async execute(value: T, model: ModelConstructor<T, U>): Promise<U> {
    if (process.env.DATABASE_TYPE === DatabaseTypes.MYSQL) {
      const response = await model.destroy({ where: value })
      return response
    }
